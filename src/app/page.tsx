...
interface HomePageData {
  puanDurumu: PuanDurumu | null;
  fikstur: FiksturHaftasi[];
  siteAyarlari: SiteAyarlari | null;
  featuredPost: Post | null; 
  recentPosts: Post[] | null; 
}

// Ana Sayfa Artık Tek ve Güçlü bir Client Component
export default function HomePage() {
  ...
  useEffect(() => {
    const query = groq`{
      ...
      "fikstur": *[_type == "fiksturHaftasi"]{
        hafta, "maclar": maclar[]{_key, tarih, skor, "evSahibi": evSahibi->{isim}, "deplasman": deplasman->{isim}}
      } | order(hafta asc),
    ...
    const fetchData = async () => {
      setLoading(true);
      const response: HomePageData = await client.fetch(query);
      setData(response);
      setSeciliHafta(response.puanDurumu?.hafta || 1);
      setLoading(false);
    };
    fetchData();
  }, []);
  ...
  const gosterilecekMaclar = data?.fikstur.find(f => f.hafta === seciliHafta)?.maclar || [];
        ...
        <>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 md:gap-12 items-start">
              <div className="lg:col-span-2">
                ...
                <div className="bg-surface rounded-lg border border-subtle-border min-h-[400px] flex items-center justify-center">
                  {gosterilecekMaclar.length > 0 ? (<div className="w-full">{gosterilecekMaclar.map(mac => (<div key={mac._key} className="flex flex-col md:flex-row items-center justify-between p-4 border-b border-subtle-border last:border-none"><div className='text-xs text-secondary-text flex items-center mb-2 md:mb-0 md:mr-4 shrink-0'><FiCalendar className="mr-2"/>{mac.tarih ? new Date(mac.tarih).toLocaleString('tr-TR', { day: 'numeric', month: 'long', hour: '2-digit', minute: '2-digit' }) : 'Tarih Belirsiz'}</div><div className="flex items-center justify-center flex-grow w-full"><span className="text-primary-text font-semibold text-base md:text-lg text-right w-2/5 truncate">{mac.evSahibi?.isim}</span><span className="text-primary-text font-bold mx-2 bg-background px-4 py-2 rounded-md text-xl">{mac.skor || '-'}</span><span className="text-primary-text font-semibold text-base md:text-lg text-left w-2/5 truncate">{mac.deplasman?.isim}</span></div></div>))}</div>) : <div className="p-8 text-center">Bu hafta için maç verisi bulunamadı.</div>}
                </div>
              </div>
              <div className="space-y-8 mt-8 lg:mt-10">
                <PuanTablosu tablo={data?.puanDurumu || undefined} />
              </div>
            </div>
          </section>
          ...
          <section className="container mx-auto px-4 md:px-6 mt-16 border-t border-subtle-border pt-16 pb-24">
            ...
            <div className="space-y-12">
              {data?.featuredPost && <FeaturedPostCard post={data.featuredPost} />}
              {data?.recentPosts && data.recentPosts.length > 0 && (<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">{data.recentPosts.map((post) => (<PostCard key={post._id} post={post} />))}</div>)}
